...
class GridTacticsGame {
    ...
    PLAYER_1_NUMBERS = ['1️⃣', '2️⃣', '3️⃣', '4️⃣']; // Jogador 1
    PLAYER_2_NUMBERS = ['①', '②', '③', '④']; // Jogador 2 (IA ou Player 2)
    constructor(logger = console, config = null) {
        this.games = new Map();
        this.logger = logger;
        this.config = config;
    }
     ...
     */
    createGame(chatId, player1, player2, difficulty = 'medio') {
     ...
     */
    validateMove(board, row, col, number, playerIndex) {
        const index = row * 4 + col;
     ...
     */
    calculateAIMove(board, difficulty) {
    ...
    minimax(board, depth, isMaximizing, alpha, beta) {
        ...
        if (winner === 1)
            return 100 + depth; // IA ganha
        if (winner === 0)
            return -100 - depth; // Humano ganha
