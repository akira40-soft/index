...
class CommandHandler {
    ...
    constructor(sock, config, bot = null, messageProcessor = null) {
        ...
        const RegistrationClass = RegistrationSystem.default || RegistrationSystem;
        const LevelClass = LevelSystem.default || LevelSystem;
        const EconomyClass = EconomySystem.default || EconomySystem;
        const MediaClass = MediaProcessor.default || MediaProcessor;
        ...
        try {
            ...
            if (isGameReply && /^\d+$/.test(msgTexto)) {
                ...
                if (gameModule) {
                    ...
                    const normSender = gameModule._normalizeId(senderId);
            ...
            const permissionCheck = this.permissionManager.canExecuteCommand(command, userId, nome, ehGrupo, groupJid);
            ...
            switch (command) {
                ...
                case 'membros': {
                    ...
                    if (!isOwner && !isAdminUsers) {
                        await this.bot.reply(m, '🚫 Apenas admins podem listar membros.');
                        return true;
                    }
                ...
                case 'sorteio': {
                    ...
                    if (!isOwner && !isAdminUsers) {
                        await this.bot.reply(m, '🚫 Apenas admins podem fazer sorteios.');
                        return true;
                    }
...
💡 Use *${P}menu [categoria]* para ver detalhes de cada comando.`;
    ...
    async _handleTakeSticker(m, args, nome) {
        try {
            ...
            const outBuf = await this.mediaProcessor.addStickerMetadata(bufInfo.buffer, nome.split(' ')[0], // Pack
    ...
    async _handlePinterest(m, query, args) {
        ...
        try {
            ...
            let images = [];
            if (jsonMatch) {
                images = jsonMatch.map((match) => {
                    const urlMatch = match.match(/url\":\"(https:\/\/i\.pinimg\.com\/originals\/[a-f0-9]+\.(jpg|png|gif))/);
                    return urlMatch ? urlMatch[1] : null;
                }).filter((url) => url !== null);
            }
            if (images.length === 0) {
                const genericMatch = response.data.match(/https:\/\/i\.pinimg\.com\/[^\/]+\/[a-f0-9]+\.(jpg|png|gif)/g);
                if (genericMatch)
                    images = [...new Set(genericMatch)];
            }
            if (images.length === 0) {
                await this._reply(m, '❌ Não consegui encontrar imagens no Pinterest no momento. Tente novamente mais tarde.');
                return true;
            }
            const uniqueImages = Array.from(new Set(images)).slice(0, count);
        ...
        switch (subCommand) {
            ...
            case 'bemvindo':
                ...
                if (args[0] === 'status') {
                    ...
                    await this._reply(m, `📝 *STATUS - BOAS-VINDAS*\n\n` +
            ...
            case 'setwelcome':
                ...
                if (!fullArgs || fullArgs.trim() === '') {
                    await this._reply(m, `📝 *Como usar #setwelcome:*\n\n` +
            ...
            case 'setgoodbye':
                // ✅ FIX: Permitir personalização quando não há args (responder com help)
                if (!fullArgs || fullArgs.trim() === '') {
                    await this._reply(m, `📝 *Como usar #setgoodbye:*\n\n` +
            ...
            case 'goodbye':
                ...
                if (args[0] === 'status') {
                    ...
                    await this._reply(m, `📝 *STATUS - DESPEDIDA*\n\n` +
