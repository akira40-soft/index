...
class PaymentManager {
    ...
    setSocket(sock) {
        // O PaymentManager usa this.bot.sock, mas adicionamos para conformidade com o sync global
        if (this.bot)
            this.bot.sock = sock;
    }
    /*
     ...
     *    - Para configurar: Edite database/payplay_config.json
     *
    ...
    _saveJSON(p, data) {
        try {
            ...
            console.error('PaymentManager save erro:', e.message);
        }
    }
    /**
    ...
    async _processKofiWebhook(data) {
        ...
        if (amount >= 30)
            days = 60;
        if (amount >= 50)
            days = 999; // Permanente se doar muito?
