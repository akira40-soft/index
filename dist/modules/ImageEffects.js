...
class ImageEffects {
    ...
    generateTempPath(ext = 'png') {
        return path.join(this.tempFolder, `effect_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`);
    }
    /**
...
`;
    ...
    */
    ...
    async applyGayEffect(imageBuffer) {
        try {
            const metadata = await sharp(imageBuffer).metadata();
            const rainbow = Buffer.from(`<svg width="${metadata.width}" height="${metadata.height}">
    ...
    async applyWastedEffect(imageBuffer) {
        try {
            ...
            const wastedSvg = Buffer.from(`<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
     ...
     */
    async applyMissionPassedEffect(imageBuffer) {
        try {
            const metadata = await sharp(imageBuffer).metadata();
            const width = metadata.width || 512;
            const height = metadata.height || 512;
            ...
            const missionSvg = Buffer.from(`<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    async applyTriggeredEffect(imageBuffer) {
        try {
            ...
            const width = metadata.width || 512;
            const height = metadata.height || 512;
            const triggeredSvg = Buffer.from(`<svg width="${width}" height="${height}">
