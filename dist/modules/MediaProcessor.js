...
class MediaProcessor {
     ...
     */
    async _getMetadataFromInvidious(videoId) {
     ...
     */
    async _getMetadataFromPiped(videoId) {
     ...
     */
    async _downloadStreamFromPiped(videoId, outputPath) {
        if (!videoId) {
        ...
        for (const instance of pipedInstances) {
            try {
                ...
                if (data.audioStreams && data.audioStreams.length > 0) {
                    ...
                    const best = sorted.find((s) => s.url);
     ...
     */
    async _downloadVideoStreamFromPiped(videoId, outputPath, targetQuality = '720') {
        if (!videoId) {
            return { sucesso: false, error: 'videoId não pode ser vazio' };
        }
        // Instâncias Piped VERIFICADAS (Março 2026)
        ...
        for (const instance of pipedInstances) {
            try {
                ...
                if (data.videoStreams && data.videoStreams.length > 0) {
                    ...
                    const validStreams = data.videoStreams.filter((s) => s.quality !== 'auto');
                    if (validStreams.length > 0) {
                        const sorted = validStreams.sort((a, b) => {
                            const qA = parseInt(a.quality.replace('p', '')) || 0;
                            const qB = parseInt(b.quality.replace('p', '')) || 0;
                            if (qA <= targetQualNum && qB > targetQualNum)
                                return -1;
                            if (qB <= targetQualNum && qA > targetQualNum)
                                return 1;
                            return qB - qA;
                        });
     ...
     */
    async _downloadViaInvidiousProxy(videoId, outputPath, type, quality = '720') {
        if (!videoId)
            return { sucesso: false, error: 'videoId vazio' };
        ...
        for (const instance of instances) {
            try {
                ...
                if (type === 'audio') {
                    ...
                    const videoStreams = data.adaptiveFormats?.filter((f) => f.type?.startsWith('video') && f.container === 'mp4') || [];
                    ...
                    const progressive = data.formatStreams?.find((f) => parseInt(f.resolution) <= targetHeight);
    ...
    async downloadMedia(message, mimeType = 'image') {
        try {
            ...
            const extractMediaContainer = (msgObj, depth = 0) => {
                if (!msgObj || typeof msgObj !== 'object' || depth > 5)
                    return null;
