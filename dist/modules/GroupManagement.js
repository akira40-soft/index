...
class GroupManagement {
    ...
    constructor(sock, config = null, moderationSystem = null, mediaProcessor = null, levelSystem = null) {
        this.sock = sock;
    ...
    async _getGroupAdmins(groupJid) {
        ...
        const admins = metadata.participants
            ...
            .map((p) => p.id ? JidUtils.normalize(p.id) : '');
     ...
     */
    async _waitForSocket(maxWaitMs = 3000) {
        return this._checkSocket();
    }
    /**
     * Obtém metadados do grupo com cache e retry
     */
    async _getGroupMetadata(groupJid, retries = 2) {
        // Verifica cache
        ...
        try {
            ...
            const admins = metadata.participants
                ...
                .map((p) => p.id ? p.id.split('@')[0].split(':')[0] + '@s.whatsapp.net' : '');
            this.adminCache.set(groupJid, { admins, timestamp: Date.now() });
            return metadata;
        }
        catch (e) {
    ...
    async sendWelcomeMessage(groupJid, participants) {
        if (!this.getWelcomeStatus(groupJid))
            return;
    ...
    async sendGoodbyeMessage(groupJid, participants) {
        if (!this.getGoodbyeStatus(groupJid))
            return;
    ...
    async formatMessage(groupJid, participantJid, template) {
        try {
            ...
            try {
                const me = metadata.participants.find((p) => p.id === this.sock?.user?.id);
     ...
     */
    async setGroupPhotoDirect(groupJid, buffer) {
        if (!this._checkSocket())
            return { success: false, error: 'Socket não disponível' };
        try {
            await this.sock.updateProfilePicture(groupJid, buffer);
     ...
     */
    async isGroupAdmin(groupJid, userNumber) {
        try {
            const admins = await this._getGroupAdmins(groupJid);
