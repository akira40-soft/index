...
class PermissionManager {
    ...
    logger;
    sock;
    constructor(logger = console, registrationSystem = null) {
    ...
    */
    isOwner(numero, nome = '') {
        try {
            ...
            const matchByNumber = this.owners?.some(owner => String(owner.numero).replace(/\D/g, '') === numeroLimpo);
            if (matchByNumber)
                return true;
    ...
    */
    containsLink(texto) {
        ...
        return this.securityConfig?.linkPatterns?.some((pattern) => textLower.includes(pattern.toLowerCase()));
    }
    /**
    ...
    */
    getNextMuteDuration(muteCount) {
        ...
        // Fórmula: 5 * 2^(n-1)
        return Math.min(baseDuration * Math.pow(multiplier, muteCount), 1440 // Máximo de 1 dia (1440 minutos)
        );
    }
    /**
